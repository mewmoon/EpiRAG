...
class MySimpleAgent(SimpleAgent):
    ...
    def _parse_tool_parameters(self, tool_name: str, parameters: str) -> dict:
        ...
        # 🌟 1. 拦截并处理大模型自作聪明包上的 {} 或者是标准的 JSON 格式
        if parameters.startswith("{") and parameters.endswith("}"):
            try:
                # 尝试当作标准 JSON 解析 (例如：{"tool_input": "15*8+32"})
                # 先把非标准 JSON（缺双引号的）修复一下，或者直接用正则提取
                cleaned_json = re.sub(
                    r"([a-zA-Z0-9_]+)\s*:", r'"\1":', parameters
                )  # 给 key 补上双引号
                cleaned_json = cleaned_json.replace("'", '"')
                return json.loads(cleaned_json)
            except Exception:
                # 如果不是标准 JSON，只是单纯被花括号包住的代码 (例如：{15 * 8 + 32})
                # 那就直接无情地把外层的 { 和 } 剥掉！
                parameters = parameters[1:-1].strip()

        # 🌟 2. 统一名称兼容性：将 python_calculator 映射到 calculator 的处理逻辑中
        if "calculator" in tool_name:
            # 计算器通常只需要里面的表达式字符串
            if ":" in parameters and not (parameters.startswith("http")):
                # 防止模型写成 tool_input:15*8+32，切分出真正的内容
                _, val = parameters.split(":", 1)
